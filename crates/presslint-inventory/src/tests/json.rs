//! Minimal in-memory JSON model and serde adapters for inventory shape tests.
//!
...
//! sibling selector/action/color harnesses, with `null` and `f64` support for
//! inventory option fields, bounds, and color components.
...
impl ser::Serializer for JsonSerializer {
    ...
    fn serialize_bool(self, value: bool) -> Result<Self::Ok, Self::Error> {
        Err(Self::Error::custom(format!(
            "unsupported bool JSON value {value}"
        )))
    }
    ...
    fn deserialize_enum<V: de::Visitor<'de>>(
        ...
        match self {
            ...
            Self::Object(fields) if fields.len() == 1 => {
                let Some((variant, value)) = fields.into_iter().next() else {
                    return Err(Self::Error::custom("missing newtype enum variant"));
                };

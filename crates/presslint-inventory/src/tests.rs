...
fn combined_inventory_classifies_shared_name_as_image() -> Result<(), String> {
    // A `Do` name present in both lists is classified as an image; the form list
    // never produces an entry for it.
    let inventory = combined_inventory(
        b"/Dup Do",
        &ContentScope::Page,
...
fn combined_inventory_entries_match_per_kind_builders_except_sequence() -> Result<(), String> {
    // Two vectors and two texts interleaved, so the global sequence diverges from
    // the per-kind sequence for the second entry of each kind.
    ...
    // Pair each combined entry with its per-kind counterpart in content order.
    let pairs = [
    ...
    // The very first emitted entry keeps global sequence 0, matching the first
    // vector's per-kind identity exactly (same sequence -> same digest).
    assert_eq!(combined.entries[0].id, vectors.entries[0].id);

    // Every later entry gets a higher global sequence than its per-kind builder
    // would assign, so only the sequence and the resulting digest differ.
